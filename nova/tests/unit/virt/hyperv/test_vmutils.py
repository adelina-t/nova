...
class VMUtilsTestCase(test.NoDBTestCase):
    ...
    def test_destroy_nic(self, mock_get_nic_data_by_name):
        ...
        with mock.patch.object(self._vmutils,
                '_remove_virt_resource') as mock_remove_virt_res:

            self._vmutils.destroy_nic(self._FAKE_VM_NAME,
                                      self._FAKE_NIC_NAME)
            mock_remove_virt_res.assert_called_with(fake_nic_data,
                self._FAKE_VM_PATH)
    ...
    def _test_get_vm_generation(self, vm_gen):
        ...
        mock_vssd = mock.Mock(VirtualSystemSubType=vm_gen_string)
        mock_vssd.VirtualSystemSubType = vm_gen_string
