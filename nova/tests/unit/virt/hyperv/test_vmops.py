...
class VMOpsTestCase(test.NoDBTestCase):
    ...
    def _test_create_instance(self, mock_attach_drive, mock_attach_volumes,
        ...
        self._vmops._vmutils.create_vm.assert_called_once_with(
            ...
            CONF.hyperv.dynamic_memory_ratio, constants.VM_GEN_1,
            [mock_instance.uuid])
        expected = []
        ctrl_type = vmops.VM_GENERATIONS_CONTROLLER_TYPES[constants.VM_GEN_1]
        ctrl_disk_addr = 0
        if fake_root_path:
    ...
    def _test_attach_interface(self,
        ...
        if (vm_generation == constants.VM_GEN_1
            ...
            self._vmops._vmutils.create_nic.assert_called_with(
                                                mock.sentinel.FAKE_VM_NAME,
                                                mock.sentinel.FAKE_VIF_ID,
                                                mock.sentinel.FAKE_VIF_ADDRESS)
            fake_vif_plug.assert_called_with(fake_vm, fake_vif)
    ...
    def _test_detach_interface(self,
        ...
        if (vm_generation == constants.VM_GEN_1
            ...
            fake_vif_unplug.assert_called_with(fake_vm, fake_vif)
            self._vmops._vmutils.destroy_nic.assert_called_with(
                                                mock.sentinel.FAKE_VM_NAME,
                                                mock.sentinel.FAKE_VIF_ID)
