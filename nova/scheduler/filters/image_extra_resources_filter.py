...
class ImageExtraResourcesFilter(filters.BaseHostFilter):
    ...
    any properties specified on the instance's image properties. They must
    have the format hypervisor:property to be verified by this filter.
    Architecture, hypervisor type and virtual machine mode are skipped by this
    filter, as they do not have the specified format and they are already
    ...
    def _satisfies_extra_resources(self, host_state, image_properties):
        ...
        for key, required in image_properties.iteritems():
            # expected format: hypervisor:property
            keys = key.split(':')
            if len(keys) <= 1:
                # skip this property.
                continue
